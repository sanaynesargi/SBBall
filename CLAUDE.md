...
- `src/lib/performanceRating.ts` — `calculateGameScore()`: a Hollinger-style **Game Score** (PTS + 0.4·FGM − 0.7·FGA + 0.7·ORB + 0.3·DRB + STL + 0.7·AST + 0.7·BLK − 0.4·PF − TOV). **Free throws are disregarded entirely**: FT makes are NOT counted in PTS (PTS = 2s·2 + 3s·3 only) and the missed-FT term is dropped (no FTA tracked). Stored in the `rating` column, displayed as "GMSC" (color tiers in `utils/rating.ts`). Replaced the old ad-hoc weight/bonus rating. `scripts/recompute-ratings.mjs` (`npm run db:recompute-ratings`) rebuilds it across all games — **keep its gameScore() in sync with this file**. 4v4 runs higher than 2v2 (more possessions).
